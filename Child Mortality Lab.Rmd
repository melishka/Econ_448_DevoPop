...
Now read the data. You should have downloaded a file called "ipumsi_00018.dat.gz" from your IPUMS account. Make sure you have both files (xml and dat.gz) and that they are in the same directory.
...
```{r}
#Make dummy variables for urban, literacy, employment, piped water, marriage type
df_analysis$URBAN_RES <- ifelse(df_analysis$URBAN == 2, 1, 0) 
df_analysis$LITERATE<-ifelse(df_analysis$LIT==2,1,0)
df_analysis$EMPLOYED<-ifelse(df_analysis$EMPSTAT==1,1,0)
df_analysis$NOPIPED<-ifelse(df_analysis$WATSUP==20,1,0)
df_analysis$HASRADIO<-ifelse(df_analysis$RADIO==2,1,0)
...
```{r}
f_ols2<-feols(CMR~i(YRSCHOOL)+CHBORN+AGE+EMPLOYED, data=df_analysis)
iplot(f_ols2)
```
...
```{r}
#install.packages("ggfixest", dependencies = TRUE)
#library(ggfixest)
